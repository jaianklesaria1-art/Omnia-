    ...
    urls.map(
      src =>
        new Promise<void>(resolve => {
          if (src.endsWith('.mp4') || src.endsWith('.webm') || src.includes('.mp4')) {
            resolve();
            return;
          }
          const img = new Image();
...
interface Item {
  id: string;
  img: string; // can be image or video URL
  url: string;
  height: number;
  video?: boolean;
}
...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useEffect(() => {
    ...
    // Safety timeout - if images take longer than 3 seconds to load or fail, show them anyway
    const fallbackTimeout = setTimeout(() => {
      if (isMounted && !imagesReady) {
        setImagesReady(true);
      }
    }, 3000);
    ...
    preloadImages(items.map(i => i.img))
      ...
      .catch(() => {
        // Fallback in case image preloading completely fails
        if (isMounted) setImagesReady(true);
      });
    ...
    <div ref={containerRef} className="list relative" style={{ minHeight: maxContentHeight > 0 ? `${maxContentHeight + 20}px` : 'auto' }}>
      ...
      {activeVideo && (
        ...
        >
          ...
          <div className="relative w-full max-w-4xl max-h-[90vh] flex items-center justify-center rounded-xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <video 
              src={activeVideo} 
              autoPlay 
              controls 
      ...
      {grid.map(item => {
        return (
          <div
            ...
            style={{ opacity: 0 }} // Hidden initially until GSAP takes over
